                            ...
                            <li>
                                @foreach($imagesGalleries as $imagesGallery)
                                    <a href="{{url('products/large',$imagesGallery->image)}}" data-standard="{{url('products/small',$imagesGallery->image)}}">
                                        <img src="{{url('products/thumbnails',$imagesGallery->image)}}" alt="" width="80" style="padding: 8px;">
                                    </a>
                                @endforeach
                            </li>
                        </ul>
                        ...
                        <form action="{{route('addToCart')}}" method="post" role="form">
                            ...
                            @foreach($detail_product->attributes as $attrs)
                                ...
                                <span>
                                    ...
                                    <input type="text" name="quantity" value="" id="inputStock"/>
                                    @if($totalStock>0)
                                                ...
                                                <div class="single-products">
                                                    <div class="productinfo text-center">
                                                        <a href="{{ route('shop.show')}}">
                                                            <img src="{{url('/products/small',$item->image)}}" alt="" style="width: 150px;"/></a>
                                                        <p>{{$item->p_name}}</p>
                                                        <h2>£{{$item->price}}</h2>
